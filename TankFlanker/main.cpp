...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	do {
		...
		for (int p_cnt = 0; p_cnt < playerc; ++p_cnt) {
		...
		for (auto&& p : player) {
			...
			p.aim = -1;
			//hit
			for (size_t i = 0; i < p.ptr->colmeshes; ++i) {
				MV1SetupCollInfo(p.colobj.get(), -1, 5, 5, 5, int(i));
			}
			...
			/*0123は装甲部分なので詰め込む*/
			p.HP[0] = 1; //life
			for (size_t i = 4; i < p.ptr->colmeshes; ++i) {
				p.HP[i] = 100;
			} //spaceARMER
			//wheel
		...
		while (ProcessMessage() == 0) {
			...
			if (GetActiveFlag() == TRUE) {
				...
				if (keyget[7]) {
					...
					if (selfammo == 1) {
						++player[0].ammotype;
						player[0].ammotype %= 3;
						player[0].loadcnt[0] = 1;
					}
				}
				else {
					if (player[0].loadcnt[0] == 0) {
						selfammo = 0;
					}
				}
				/*指揮*/
				if (map.flug) {
					...
					for (size_t p_cnt = 1; p_cnt < teamc; ++p_cnt) {
						if (player[p_cnt].HP[0] > 0) {
							if (inm(x_r(132), y_r(162 + p_cnt * 24), x_r(324), y_r(180 + p_cnt * 24))) {
								choose = p_cnt;
			...
			if (true) {
						...
						if (p.id == 0) {
							...
							*/
							...
							if (!p.atkf) {
								...
								cpu_move = tempvec[1].y * std::hypot(tempvec[0].x, tempvec[0].z) - std::hypot(tempvec[1].x, tempvec[1].z) * tempvec[0].y; //*砲
				...
				for (auto& p : player) {
					...
					//common
					for (int i = bone_trt; i < bone_wheel; ++i) {
						if (i == bone_trt) {
						...
						else if (i == p.ptr->gunframe[i != bone_gun1]) {
							auto mtemp = MMult(MMult(MGetRotX(p.gunrad.y), MGetTranslate(VSub(p.ptr->loc[i], p.ptr->loc[bone_trt]))), p.ps_t);
							...
							mtemp = MGetTranslate(VAdd(VSub(p.ptr->loc[i + 1], p.ptr->loc[i]), VGet(0, 0, p.fired[i != bone_gun1])));
							MV1SetFrameUserLocalMatrix(p.obj.get(), i + 1, mtemp);
							MV1SetFrameUserLocalMatrix(p.farobj.get(), i + 1, mtemp);
							if (i == bone_gun1)
								MV1SetFrameUserLocalMatrix(p.colobj.get(), i + 1, mtemp);
						}
					}
					for (int i = bone_wheel; i < p.ptr->frames; ++i) {
						if (i < p.ptr->frames - 4) {
							if ((i - bone_wheel) % 2 == 1) {
								...
								const auto HitPoly = mapparts->get_gnd_hit(VAdd(p.obj.frame(i), VScale(p.nor, 1.0f)), VAdd(p.obj.frame(i), VScale(p.nor, -0.2f)));
								if (HitPoly.HitFlag)
									p.Springs[i] = std::min<float>(p.Springs[i] + 1.0f / f_rates, 1.0f - VSize(VSub(HitPoly.HitPosition, VAdd(p.obj.frame(i), VScale(p.nor, 1.0f)))));
								else
									p.Springs[i] = std::max<float>(p.Springs[i] - 0.2f / f_rates, -0.2f);
					...
					/*collition*/
					for (size_t i = 0; i < p.ptr->colmeshes; ++i) {
						MV1RefreshCollInfo(p.colobj.get(), -1, int(i));
					}
					/*反動*/
					if (p.loadcnt[0] > 0) {
						if (p.firerad < 180) {
					...
					for (size_t guns = 0; guns < gunc; ++guns) {
						if (p.fired[guns] >= 0.01f)
							p.fired[guns] *= 0.95f;
						if (p.loadcnt[guns] == 0) {
							if ((p.move & (KEY_SHOTCAN << guns)) != 0) {
								const auto j = p.useammo[guns] + guns * ammoc;
								p.Ammo[j].flug = true;
								p.Ammo[j].speed = p.ptr->gun_speed[p.ammotype] / f_rates;
								p.Ammo[j].pene = p.ptr->pene[p.ammotype];
								p.Ammo[j].pos = p.obj.frame(p.ptr->gunframe[guns]);
								p.Ammo[j].repos = p.Ammo[j].pos;
								p.Ammo[j].cnt = 0;

								tempvec[0] = VSub(p.obj.frame(p.ptr->gunframe[guns] + 1), p.Ammo[j].pos);
								const auto y = atan2(tempvec[0].x, tempvec[0].z) + deg2rad((float)(-1000 + GetRand(2000)) / 10000.f);
								const auto x = atan2(-tempvec[0].y, std::hypot(tempvec[0].x, tempvec[0].z)) - deg2rad((float)(-1000 + GetRand(2000)) / 10000.f);
								p.Ammo[j].vec = VGet(cos(x) * sin(y), -sin(x), cos(x) * cos(y));
								//
								p.useammo[guns]++;
								p.useammo[guns] %= ammoc;
								++p.loadcnt[guns];
								if (guns == 0) {
							...
							++p.loadcnt[guns];
							if (p.loadcnt[guns] >= p.ptr->reloadtime[guns]) {
								p.loadcnt[guns] = 0;
								if (p.id == 0 && guns == 0) {
						...
						for (size_t i = guns * ammoc; i < ammoc + guns * ammoc; ++i) {
							if (p.Ammo[i].flug) {
								...
								const auto HitPoly = mapparts->get_gnd_hit(p.Ammo[i].repos, p.Ammo[i].pos);
								if (HitPoly.HitFlag) {
									p.Ammo[i].pos = HitPoly.HitPosition;
								}
								...
								p.Ammo[i].vec.y += M_GR / 2.0f / f_rates / f_rates;
								p.Ammo[i].pene -= 1.0f / f_rates;
								p.Ammo[i].speed -= 5.f / f_rates;
								p.Ammo[i].cnt++;
								if (p.Ammo[i].cnt > (f_rates * 3.f) || p.Ammo[i].speed <= 0.f)
									p.Ammo[i].flug = false; //3秒で消える
							}
						}
					}
					if (p.recoadd) {
				...
				for (auto& p : player) {
					...
					p.effcs[ef_smoke2].handle.SetPos(p.obj.frame(bone_smoke1));
					//SetTargetLocation(p.effcs[ef_smoke2].handle, p.obj.frame(bone_smoke1));
					p.effcs[ef_smoke3].handle.SetPos(p.obj.frame(bone_smoke2));
					//SetTargetLocation(p.effcs[ef_smoke3].handle, p.obj.frame(bone_smoke2));
				}
				UpdateEffekseer3D();
			}
			/*視点*/
			...
			if (map.flug) {
				...
				for (size_t p_cnt = 0; p_cnt < teamc; ++p_cnt) {
					...
					//進軍パラメータ
					for (size_t i = 0; i < player[p_cnt].wayselect; i++) {
						DrawBox(x_r(348 + i * 12), y_r(162 + player[p_cnt].id * 24), x_r(356 + i * 12), y_r(180 + player[p_cnt].id * 24), c_3232ff, TRUE);
					}
				}
				//敵
				for (size_t p_cnt = teamc; p_cnt < playerc; ++p_cnt) {
				...
				if (parts->get_in() || aim.flug) {
					...
					for (auto& tt : pssort) {
						...
						for (int i = 1; i < player[j].ptr->meshes; ++i) {
							if (player[j].HP[i + 4] > 0) {
								MV1DrawMesh(player[j].farobj.get(), i);
							}
						}
					}
					ShadowMap_DrawEnd();
					...
					for (auto& tt : pssort) {
						...
						for (int i = 1; i < player[j].ptr->meshes; ++i) {
							if (player[j].HP[i + 4] > 0) {
								MV1DrawMesh(player[j].obj.get(), i);
							}
						}
					}
					ShadowMap_DrawEnd();
					...
					for (auto& tt : pssort) {
						...
						if (j != 0 || (j == 0 && !aim.flug)) {
							if (tt.second < 200.f || aim.flug) {
								...
								for (int i = 1; i < player[j].ptr->meshes; ++i) {
									if (i < 3) {
										MV1SetFrameTextureAddressTransform(player[j].obj.get(), 0, 0.0, player[j].wheelrad[i], 1.0, 1.0, 0.5, 0.5, 0.0);
									}
									if (i == 3) {
										MV1ResetFrameTextureAddressTransform(player[j].obj.get(), 0);
									}
									if (player[j].HP[i + 4] > 0) {
										MV1DrawMesh(player[j].obj.get(), i);
									}
								...
								for (int i = 1; i < player[j].ptr->meshes; ++i) {
									if (i < 3) {
										MV1SetFrameTextureAddressTransform(player[j].farobj.get(), 0, 0.0, player[j].wheelrad[i], 1.0, 1.0, 0.5, 0.5, 0.0);
									}
									if (i == 3) {
										MV1ResetFrameTextureAddressTransform(player[j].farobj.get(), 0);
									}
									if (player[j].HP[i + 4] > 0) {
										MV1DrawMesh(player[j].farobj.get(), i);
									}
					...
					for (auto& p : player) {
						for (size_t i = 0; i < ammoc; ++i) {
							if (p.Ammo[i].flug) {
								SetDrawBlendMode(DX_BLENDMODE_ALPHA, (int)(255.f * std::min<float>(1.f, 4.f * p.Ammo[i].speed / (p.ptr->gun_speed[p.ammotype] / f_rates))));
			...
			if (map.flug) {
				...
				/*UI*/
				if (aim.flug) {
					uiparts->draw_sight(aims.x, aims.y, rat_r, aimm, parts->get_font(0));
				} /*照準器*/
				else {
					for (auto& tt : pssort) {
