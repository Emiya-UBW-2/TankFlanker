...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	VECTOR_ref campos, viewpos, uppos;			      /*カメラ*/
	int mapc;
	//init----------------------------------------------------------------------------------//
	...
	do {
		...
		for (auto&& p : player) {
			...
			//ypos反映
			{
				...
				for (auto&& w : p.waypos) {
					...
					if (HitPoly.HitFlag)
						w = VGet(w.x(), HitPoly.HitPosition.y, w.z());
				}
			}
			//
		}
				...
				p.foot[LR] = new b2World(b2Vec2(0.0f, 0.0f));
				{
					...
					for (auto& w : p.ptr->upsizeframe) {
						...
						if (vects.x() * ((LR == 0) ? 1.f : -1.f) > 0) {
							...
							b2FixtureDef f_fixtureDef; /*動的ボディフィクスチャを定義します。*/
							f_fixtureDef.shape = &f_dynamicBox;
							f_fixtureDef.density = 20.0f;
							f_fixtureDef.friction = 0.2f;
							b2BodyDef f_bodyDef; /*ダイナミックボディを定義します。その位置を設定し、ボディファクトリを呼び出します。*/
							f_bodyDef.type = b2_dynamicBody;
							f_bodyDef.position.Set(vects.z(), vects.y());
							p.Foot[LR][i].fbody.reset(p.foot[LR]->CreateBody(&f_bodyDef));
							p.Foot[LR][i].fplayerfix = p.Foot[LR][i].fbody->CreateFixture(&f_fixtureDef); // シェイプをボディに追加します。
					...
					for (auto& w : p.ptr->wheelframe) {
						...
						if (vects.x() * ((LR == 0) ? 1.f : -1.f) > 0) {
							...
							b2FixtureDef fw_fixtureDef; /*動的ボディフィクスチャを定義します。*/
							fw_fixtureDef.shape = &shape;
							fw_fixtureDef.density = 1.0f;
							b2BodyDef fw_bodyDef; /*ダイナミックボディを定義します。その位置を設定し、ボディファクトリを呼び出します。*/
							fw_bodyDef.type = b2_kinematicBody;
							fw_bodyDef.position.Set(vects.z(), vects.y());
							p.Fwheel[LR][i].fbody.reset(p.foot[LR]->CreateBody(&fw_bodyDef));
							p.Fwheel[LR][i].fplayerfix = p.Fwheel[LR][i].fbody->CreateFixture(&fw_fixtureDef);
							++i;
						}
					}
					i = 0;
					for (auto& w : p.ptr->youdoframe) {
						vects = VTransform(VGet(0, 0, 0), MV1GetFrameLocalMatrix(p.obj.get(), w));
						if (vects.x() * ((LR == 0) ? 1.f : -1.f) > 0) {
							p.Fyudo[LR].resize(i + 1);
							b2CircleShape shape;
							shape.m_radius = 0.1f;
							b2FixtureDef fy_fixtureDef; /*動的ボディフィクスチャを定義します。*/
							fy_fixtureDef.shape = &shape;
							fy_fixtureDef.density = 1.0f;
							b2BodyDef fy_bodyDef; /*ダイナミックボディを定義します。その位置を設定し、ボディファクトリを呼び出します。*/
							fy_bodyDef.type = b2_kinematicBody;
							fy_bodyDef.position.Set(vects.z(), vects.y());
							p.Fyudo[LR][i].fbody.reset(p.foot[LR]->CreateBody(&fy_bodyDef));
							p.Fyudo[LR][i].fplayerfix = p.Fyudo[LR][i].fbody->CreateFixture(&fy_fixtureDef);
							++i;
						}
					}
				}
		...
		while (ProcessMessage() == 0) {
			...
			if (true) {
				...
				for (auto&& p : player) {
					...
					if (p.HP[0] > 0) {
						...
						if (p.id == 0) {
							...
							//*CPU操作
							//*移動
							if ((p.pos - p.waypos[p.waynow]).size() >= 10.0) {
								p.move |= KEY_GOFLONT;
								VECTOR_ref tempv = (p.waypos[p.waynow] - p.pos).Norm();
								if ((-cos(p.yrad) * tempv.x() + sin(p.yrad) * tempv.z()) < 0)
									p.move |= KEY_GOLEFT_;
								else
									p.move |= KEY_GORIGHT;
							}
				...
				for (auto& p : player) {
					...
					if ((p.move & KEY_GOFLONT) != 0 && p.gear > 0)
						p.speed = std::min<float>(p.speed + p.ptr->speed_flont[p.gear - 1] / (5.0f * f_rates), p.ptr->speed_flont[p.gear - 1]);
					if ((p.move & KEY_GOBACK_) != 0 && p.gear < 0) {
						p.speed = std::max<float>(p.speed + p.ptr->speed_back[-p.gear - 1] / (5.0f * f_rates), p.ptr->speed_back[-p.gear - 1]);
					}
					//旋回
					{
						float turn_bias = 0.f;
					...
					if (p.HP[5] == 0 || p.HP[6] == 0) {
						p.vec = VGet(
						    p.ptr->loc[p.ptr->wheelframe[0]].x() * sin(p.yadd) * sin(p.yrad) * ((p.HP[5] == 0) - (p.HP[6] == 0)),
						    0.f,
						    p.ptr->loc[p.ptr->wheelframe[0]].x() * sin(p.yadd) * cos(p.yrad) * ((p.HP[6] == 0) - (p.HP[5] == 0)));
					}
					else {
						p.vec = VGet(p.speed * sin(p.yrad), 0.f, -p.speed * cos(p.yrad));
					}
					//
				...
				for (auto& p : player) {
					...
					p.speed = VSize(
					    VGet(
						p.body->GetLinearVelocity().x,
						0,
						p.body->GetLinearVelocity().y));
					*/
					/*
					p.vec = VGet(
					    p.body->GetLinearVelocity().x,
					    p.vec.y(),
					    p.body->GetLinearVelocity().y); //使ってない
					p.vec = VGet(
					    p.speed * sin(p.yrad),
					    0.f,
					    -p.speed * cos(p.yrad));
					...
					b2Vec2 tmpb2 = b2Vec2(
					    (M_GR / 10.f) * VDot(VGet(0, -1.f, 0), (p.obj.frame(7 + 1) - p.obj.frame(7)).Norm()),
					    (M_GR / 10.f) * (p.nor % VGet(0, 1.f, 0)));
					...
					for (size_t LR = 0; LR < 2; ++LR) {
						...
						for (auto& f : p.Foot[LR])
							f.fp = VGet(f.fp.x(), f.fbody->GetPosition().y, f.fbody->GetPosition().x);
					}
				}
				/*砲撃その他*/
				for (auto& p : player) {
					//地形判定
					{
						...
						if (HitPoly.HitFlag) {
							...
							/*speed*/
							if (p.speed < ((-0.5f / 3.6f) / (5.0f * f_rates)) && (p.move & KEY_GOBACK_) == 0)
								p.speed *= 0.95f;
							if (p.speed > ((0.5f / 3.6f) / (5.0f * f_rates)) && (p.move & KEY_GOFLONT) == 0)
								p.speed *= 0.95f;
							if (abs(p.speed) < ((0.5f / 3.6f) / (5.0f * f_rates)) && (p.move & KEY_GOBACK_) == 0 && (p.move & KEY_GOFLONT) == 0)
								p.speed *= 0.95f;
							/*turn*/
					...
					for (auto& w : p.ptr->wheelframe) {
						...
						if (HitPoly.HitFlag)
							p.Springs[w] = std::min<float>(p.Springs[w] + 1.0f / f_rates, 1.0f - (VECTOR_ref(HitPoly.HitPosition) - p.obj.frame(w) - p.nor.Scale(1.0f)).size());
						else
					...
					for (size_t LR = 0; LR < 2; ++LR) {
						...
						for (auto& w : p.ptr->upsizeframe) {
							...
							if (xw * ((LR == 0) ? 1.f : -1.f) > 0) {
								p.Foot[LR][i].fp = VGet(xw, p.Foot[LR][i].fp.y(), p.Foot[LR][i].fp.z());
								MV1SetFrameUserLocalMatrix(p.obj.get(), w, p.Foot[LR][i].fp.Mtrans());
								++i;
							}
						}
					}
					...
					for (size_t guns = 0; guns < gunc; ++guns) {
						...
						for (auto& c : p.Gun[guns].Ammo)
							if (c.flug) {
								...
								c.vec = VGet(c.vec.x(), c.vec.y() + M_GR / 2.0f / f_rates / f_rates, c.vec.z());
								c.pene -= 1.0f / f_rates;
								c.speed -= 5.f / f_rates;
								c.cnt++;
			...
			if (aim.flug) {
				...
				float getdists;
				getdist(viewpos, (player[0].obj.frame(player[0].ptr->gunframe[0] + 1) - viewpos).Norm(), aim_r, getdists, player[0].ptr->gun_speed[player[0].ammotype], f_rates);
				uppos = player[0].nor;
			}
			else {
				if (!parts->get_in()) {
					if (keyget[19]) {
						...
						campos = player[0].pos + parts->get_view_pos() + VGet(0, 2, 0);
						viewpos = player[0].pos + VGet(0, 4, 0);
						const auto HitPoly = mapparts->get_gnd_hit(campos.get(), viewpos.get());
						if (HitPoly.HitFlag)
			...
			mapparts->set_camerapos(campos.get(), viewpos.get(), uppos.get(), rat_r);
			/*shadow*/
			...
			/*main*/
				...
				if (aim.flug) {
					...
					getdist(v, (p.obj.frame(p.ptr->gunframe[0] + 1) - v).Norm(), aim_r, aimm, p.ptr->gun_speed[p.ammotype], f_rates);
					aims = ConvWorldPosToScreenPos(v.get());
				}
				//pos
				...
				for (auto& p : player) {
					...
					if (CheckCameraViewClip_Box((p.pos + VGet(-5, 0, -5)).get(), (p.pos + VGet(5, 3, 5)).get()) == TRUE)
						...
						pssort[p.id] = pair(p.id, (p.pos - campos).size());
				}
						...
						for (size_t guns = 0; guns < gunc; ++guns)
							for (size_t i = 0; i < ammoc; ++i)
								if (p.Gun[guns].Ammo[i].flug) {
									...
									DrawCapsule3D(p.Gun[guns].Ammo[i].pos.get(), p.Gun[guns].Ammo[i].repos.get(), p.ptr->ammosize[guns] * ((p.Gun[guns].Ammo[i].pos - campos).size() / 60.f), 4, p.Gun[guns].Ammo[i].color, c_ffffff, TRUE);
								}
		...
		for (auto& p : player) {
			...
			for (size_t LR = 0; LR < 2; ++LR) {
				for (auto& f : p.Foot[LR]) {
					delete f.fplayerfix->GetUserData();
					f.fplayerfix->SetUserData(NULL);
				}
				for (auto& f : p.Fwheel[LR]) {
					delete f.fplayerfix->GetUserData();
					f.fplayerfix->SetUserData(NULL);
				}
				for (auto& f : p.Fyudo[LR]) {
					delete f.fplayerfix->GetUserData();
					f.fplayerfix->SetUserData(NULL);
				}
			}
